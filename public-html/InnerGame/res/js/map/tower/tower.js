...
class Tower extends Updateable {
  constructor(game, location) {
    ...
    this.game = game;
    this.cloc = location;
    this.loc = new Vector2D((this.cloc.x+.5) * config.tile_size, (this.cloc.y+.5) * config.tile_size);
  ...
  render() {
    if(this.game.mapManager.map[this.cloc.x][this.cloc.y].seen){
      ...
      this.game.context.fillText(this.type.name, this.loc.x - config.tile_size/2, this.loc.y);
      this.game.context.fillStyle = 'rgba(255,0,0,1)';
      this.game.context.fillRect(this.loc.x - config.tile_size/2, this.loc.y + 3 * config.tile_size / 10, config.tile_size, config.tile_size / 5);
      this.game.context.fillStyle = 'rgba(0,255,0,1)';
      this.game.context.fillRect(this.loc.x - config.tile_size/2, this.loc.y + 3 * config.tile_size / 10, this.hp / this.maxhp * config.tile_size, config.tile_size / 5);
    }
  }
